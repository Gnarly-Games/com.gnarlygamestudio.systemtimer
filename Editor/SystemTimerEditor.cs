...
namespace Gnarly.Timer.Editor
{
    ...
    [InitializeOnLoad]
    public static class SystemTimerEditor
    {
        public const string DayOffsetKey = "SystemTimer.DayOffset";
        public const string HourOffsetKey = "SystemTimer.HourOffset";
        public const string MinuteOffsetKey = "SystemTimer.MinuteOffset";

        static SystemTimerEditor()
        {
            SystemTimer.DayOffset = EditorPrefs.GetInt(DayOffsetKey, 0);
            SystemTimer.HourOffset = EditorPrefs.GetInt(HourOffsetKey, 0);
            SystemTimer.MinuteOffset = EditorPrefs.GetInt(MinuteOffsetKey, 0);
            ToolbarExtender.LeftToolbarGUI.Add(OnDrawSystemTime);
        }

        public static void Save()
        {
            EditorPrefs.SetInt(DayOffsetKey, SystemTimer.DayOffset);
            EditorPrefs.SetInt(HourOffsetKey, SystemTimer.HourOffset);
            EditorPrefs.SetInt(MinuteOffsetKey, SystemTimer.MinuteOffset);
        }

        private static void OnDrawSystemTime()
        {
            GUILayout.Space(20);

            if (GUILayout.Button(
                    $"{SystemTimer.Now.ToString("MM/dd/yyyy hh:mm")} - {SystemTimer.Now.DayOfWeek}",
